%%% v2 Lindsay 20230715
%%%%%%%%%%%%%% (1) change photometry signal to z score 
%%%%%%%%%%%%%% (2) correct unit of labels_timecrop (when it's in min, change it to hour)
%%%%%%%%%%%%%% (3) make the analysis to go through individual folders automatically
%%%%%%%%%%%%%% (4) sometimes labels length < labels_timecrop length. Fix the bug
close all;
clear all;

dirlist1 = dir(pwd);
isub = [dirlist1(:).isdir];
mouseList = {dirlist1(isub).name}';
mouseList(1:2)=[];            %% delete '.' and '..'

for m_id=1:length(mouseList)
    disp(mouseList{m_id});
    cd(mouseList{m_id});
    dirlist2 = dir(pwd);
    isub = [dirlist2(:).isdir];
    sessionList = {dirlist2(isub).name}';
    sessionList(1:2)=[];            %% delete '.' and '..'

    for s_id = 1:length(sessionList)
        disp(sessionList{s_id});
        cd(sessionList{s_id});
        clearvars -except mouseList m_id sessionList s_id;
...
if length(labels) ~=length(labels_timecrop)
labels(end+1)=labels(end);
end
        %%
        labels = labels.';
        if max(phot_timehourcrop)>60
            phot_timehourcrop = phot_timehourcrop/60; %% old analysis, data saved in unit of minutes
            labels_timecrop = labels_timecrop/60; %% old analysis, data saved in unit of minutes
        end
        z_score = (photometry - mean(photometry))/std(photometry);           % z - score
        ...
        for i = 1:length(state_change_frame)
            ...
            switch cur_nest_state
                case 1
                    in_phot_all = horzcat(in_phot_all , z_score(phot_start_ind:phot_end_ind));

                    switch cur_label
                        case 1
                            in_1_all = horzcat(in_1_all,z_score(phot_start_ind:phot_end_ind));
                        case 2
                            in_2_all = horzcat(in_2_all,z_score(phot_start_ind:phot_end_ind));
                        case 3
                            in_3_all = horzcat(in_3_all,z_score(phot_start_ind:phot_end_ind));
                    end

                case 0
                    out_phot_all = horzcat(out_phot_all , z_score(phot_start_ind:phot_end_ind));

                    switch cur_label
                        case 1
                            out_1_all = horzcat(out_1_all,z_score(phot_start_ind:phot_end_ind));
                        case 2
                            out_2_all = horzcat(out_2_all,z_score(phot_start_ind:phot_end_ind));
                        case 3
                            out_3_all = horzcat(out_3_all,z_score(phot_start_ind:phot_end_ind));
                    end
        ...
        se1 = std(in_phot_all)/sqrt(numel(in_phot_all));
        se2 = std(out_phot_all)/sqrt(numel(out_phot_all));
        ...
        se1 = std(out_1_all)/sqrt(numel(out_1_all));
        se2 = std(out_2_all)/sqrt(numel(out_2_all));
        se3 = std(out_3_all)/sqrt(numel(out_3_all));
        ...
        saveas(gcf, [name '_zscore_ana.png']);
        ...
        zscore_ana = array2table(data,'VariableNames',variables);
        save([name '_zscore_ana.mat'], 'zscore_ana');

        parentDir = fullfile(pwd, '..');  % navigate to the upper folder level (parent directory) -> individual mouse folder
        cd(parentDir);
    end
    parentDir = fullfile(pwd, '..');  % navigate to the upper folder level (parent directory) -> folder consisting all mouse folders
    cd(parentDir);
end
