...
def set_a_cluster(Dataframe,name,a1_max,a2_min):
    ...
    Dataframe[name+'_a_c'] = np.where(abs(Dataframe[name+'_a']) < a1_max, 1, np.where(abs(Dataframe[name+'_a']) > a2_min, 2, 0))
...
def laser_a_cluster(Dataframe,name,dataname,re_gen_plots=False):
    ...
    if not os.path.exists(dataname + '_cluster1.pickle') \
        ...
        # Load laser file
        laser_fname = new_file = dataname + '.mat'
        if not os.path.exists(laser_fname):
            print('Waiting for laser file selection...')
            file_path = askopenfilename(filetypes=[("MATLAB Files", "*.mat")])
        ...
        plt.plot(x, np.mean(np.array(c_all), axis=0), color='black')
        ...
        if len(c2) != 0:
            plt.plot(x, np.mean(np.array(c2), axis=0), color='blue')
